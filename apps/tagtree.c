...
/* "Go to Album" / "Go to Artist" navigation from WPS context menu */

/* Storage for goto lists */
...
/* Callback for goto list */
static const char* goto_list_get_name(int selected_item, void *data,
                                      char *buffer, size_t buffer_len)
{
    (void)data;
    (void)buffer;
    (void)buffer_len;
    if (selected_item >= 0 && selected_item < goto_data.count)
        return goto_data.items[selected_item];
    return "";
}

/* Navigate to album tracks for currently playing track */
bool tagtree_goto_current_album(void)
{
    ...
    /* Save album/artist info for later use */
    strmemccpy(goto_data.title, id3->album, sizeof(goto_data.title));
    if (id3->albumartist)
        strmemccpy(goto_data.artist, id3->albumartist, sizeof(goto_data.artist));
    else
        goto_data.artist[0] = '\0';
    ...
    clause.numeric = false;
    clause.source = source_constant;
    clause.str = goto_data.title;
    tagcache_search_add_clause(&tcs, &clause);

    /* Also filter by albumartist if available */
    if (goto_data.artist[0])
    {
        struct tagcache_search_clause artist_clause;
        memset(&artist_clause, 0, sizeof(artist_clause));
        artist_clause.tag = tag_albumartist;
        artist_clause.type = clause_is;
        artist_clause.numeric = false;
        artist_clause.source = source_constant;
        artist_clause.str = goto_data.artist;
        tagcache_search_add_clause(&tcs, &artist_clause);
    }

    while (tagcache_get_next(&tcs, buf, sizeof(buf)) && goto_data.count < GOTO_MAX_ITEMS)
    {
    ...
    /* Show track list */
    struct simplelist_info info;
    simplelist_info_init(&info, goto_data.title, goto_data.count, NULL);
    info.get_name = goto_list_get_name;

    if (simplelist_show_list(&info))
        return true;  /* USB connected */

    if (info.selection < 0)
        return true;  /* User pressed back */

    /* User selected a track - create playlist and play from that position */
    splash(0, ID2P(LANG_WAIT));
    ...
    if (count > 0)
        playlist_start(info.selection, 0, 0);

    return true;
}

/* Navigate to artist's albums/tracks for currently playing track */
bool tagtree_goto_current_artist(void)
{
    ...
    {
        strmemccpy(goto_data.artist, id3->albumartist, sizeof(goto_data.artist));
        goto_data.artist_tag = tag_albumartist;
    }
    else if (id3->artist)
    {
        strmemccpy(goto_data.artist, id3->artist, sizeof(goto_data.artist));
        goto_data.artist_tag = tag_artist;
    }
    else
        return false;
    ...
    /* Show album list */
    struct simplelist_info info;
    simplelist_info_init(&info, goto_data.artist, goto_data.count, NULL);
    info.get_name = goto_list_get_name;
    ...
    while (true)
    {
        ...
        else
        {
            ...
            album_clause.str = selected_album;
            tagcache_search_add_clause(&tcs, &album_clause);

            /* Also filter by artist */
            memset(&clause, 0, sizeof(clause));
            clause.tag = goto_data.artist_tag;
            clause.type = clause_is;
            clause.numeric = false;
